...
helper.sendGetRequest = function (uri, configId, cb) {
  helper.sendGetQSKeyRequest(uri, null, configId, 'restKey', cb);
}

// Simple version to use with null query string
helper.sendGetKeyRequest = function (uri, configId, keyType, cb) {
  helper.sendGetQSKeyRequest(uri, null, configId, keyType, cb);
}

helper.sendPutOrPost = function(uri, method, body, configId, keyType, cb) {
  // This gets either the restKey or adminKey
  var localKey = config.get(configId + '.' + keyType);
